"""
Modified version of the batch normalization that can be found in the e3nn library (https://github.com/e3nn/e3nn)
Implementation by Thomas Wimmer (https://github.com/wimmerth)
"""
import torch
...
class BatchNorm(nn.Module):
    def __init__(self, Rs, eps=1e-5, momentum=0.1, affine=True, reduce='mean', normalization='component'):
        """
        Batch normalization layer for orthonormal representations in 6D dMRI scans
        ...
        input shape : [batch, , stacked orthonormal representations, [spacial dimensions]]
    ...
    def forward(self, input):
        """
        :param input: [batch, stacked features, q, x, y, z]
        """
        input = input.permute(0, 2, 3, 4, 5, 1)
        ...
        for m, d in self.Rs:
            field = input[:, :, ix: ix + m * (2 * (d - 1) + 1)]  # [batch, sample, mul * repr]
            ix += m * (2 * (d - 1) + 1)

            # [batch, sample, mul, repr]
            field = field.reshape(batch, -1, m, (2 * (d - 1) + 1))
            ...
            fields.append(field.reshape(batch, -1, m * (2 * (d - 1) + 1)))  # [batch, sample, mul * repr]
        ...
        output = output.reshape(batch, *size, dim)
